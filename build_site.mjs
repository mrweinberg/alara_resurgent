...
function parseDesignBible(filePath) {
  ...
  let currentBuffer = []; 
  let parsingSection = "INTRO"; // INTRO, FLAVOR, MECHANICS, CARDS
...
function generateHTML(cards, setInfo) {
  ...
  const mechanicsHTML = setInfo.mechanics.map(line => {
      // Bold the mechanic name (e.g., "* Fabricate N (Esper - WUB):")
...
<head>
  ...
  <style>
    ...
    /* FLEX HEADER - NO WRAP */
    .card-header { display: flex; justify-content: space-between; align-items: center; gap: 8px; margin-bottom: 8px; font-weight: bold; font-size: 1.1em; white-space: nowrap; }
    .card-name { flex: 1; overflow: hidden; text-overflow: ellipsis; min-width: 0; }
    .card-cost { flex: 0 0 auto; text-align: right; }
